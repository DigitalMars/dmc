 ...
 *  pszText         LPSTR to the text for edits, listboxes, combobox, and text.
 ...
 *
 * Return Value:
 *  PGIZMO          If NULL returned then GizmoPAllocate could not allocate
...
PGIZMO GizmoPAllocate(LPINT pfSuccess, PPGIZMO ppFirst, HWND hWndParent
	, UINT iType, UINT iGizmo, UINT uID, UINT dx, UINT dy, LPSTR pszText
	...
	*pfSuccess=FALSE;
	...
	pGizmo=(PGIZMO)(char *)LocalAlloc(LPTR, CBGIZMO);
	...
	/*
	 * If this is new gizmo is a window, create it.  There's proabably
	 * a better way to write this code.  Gen, got any ideas?
	 */
	switch (iType)
			{
			case GIZMOTYPE_TEXT:
	...
	if (GIZMOTYPE_WINDOWS & iType)
		...
		pGizmo->hWnd=CreateWindow(pszClass, pszText, dwStyle | WS_CHILD | WS_VISIBLE
			, pGizmo->x, pGizmo->y, dx, pGizmo->dy, hWndParent
			, (HMENU)uID, hInst, NULL);
 ...
 *
 * Return Value:
 *  PGIZMO          NULL if successful, pGizmo if not, meaning we couldn't
 *                  free something.
 */
 ...
 * Parameters:
 *  ppFirst         PPGIZMO providing the first gizmo in this list.
 *  pfnEnum         PFNGIZMOENUM to call for each enumerated structure.
 *  dw              DWORD extra data to pass to the enumeration function.
 *
 * Return Value:
 *  PGIZMO          NULL if the enumeration completed.  Otherwise a pointer
