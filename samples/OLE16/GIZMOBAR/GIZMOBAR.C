...
LRESULT FAR PASCAL GizmoBarWndProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
	...
	switch (iMsg)
...
#else
			if ((WPARAM)NULL!=wParam && IsGDIObject((HFONT)wParam))
#endif
				{
		...
		case WM_MOUSEMOVE:
			...
			fTemp=pGB->fMouseOut;
			pGB->fMouseOut=!PtInRect(&rc, point);
...
BOOL FAR PASCAL FEnumChangeFont(PGIZMO pGizmo, UINT iGizmo, DWORD dw)
	{
...
BOOL FAR PASCAL FEnumEnable(PGIZMO pGizmo, UINT iGizmo, DWORD dw)
	{
	PGIZMOBAR   pGB=(PGIZMOBAR)dw;
...
BOOL FAR PASCAL FEnumHitTest(PGIZMO pGizmo, UINT iGizmo, DWORD dw)
	{
	RECT  rc;
	 POINT point;
#ifdef  WIN32
	 point.x = (LONG)(SHORT)LOWORD(dw);
	 point.y = (LONG)(SHORT)HIWORD(dw);
#else
	 point.x = LOWORD(dw);
	 point.y = HIWORD(dw);
#endif
