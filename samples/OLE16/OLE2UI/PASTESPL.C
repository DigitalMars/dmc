...
BOOL FPasteSpecialInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
{
	...
	n = LoadString(ghInst, (UINT)IDS_PSUNKNOWNTYPE, lpPS->szUnknownType, PS_UNKNOWNSTRLEN);
	if (n)
		n = LoadString(ghInst, (UINT)IDS_PSUNKNOWNSRC, lpPS->szUnknownSource, PS_UNKNOWNSTRLEN);
	if (!n)
	{
	...
	if (lpPS->hObjDesc = OleStdFillObjectDescriptorFromData(lpOPS->lpSrcDataObj, &medium, &cfFormat))
	{
		...
		if (lpOD->dwFullUserTypeName)
			lpPS->szFullUserTypeNameOD = (LPSTR)lpOD+lpOD->dwFullUserTypeName;
		...
		if (0==OleStdGetAuxUserType(&lpPS->clsidOD, 3, lpPS->szAppName, OLEUI_CCHKEYMAX, NULL))
		{
			 // Use "the application which created it" as the name of the application
			 if (0==LoadString(ghInst, (UINT)IDS_PSUNKNOWNAPP, lpPS->szAppName, PS_UNKNOWNSTRLEN))
			 {
				 PostMessage(hDlg, uMsgEndDialog, OLEUI_ERR_LOADSTRING, 0L);
				 return FALSE;
			 }
			...
			&medium))
	{
		...
		if (lpLSD->dwFullUserTypeName)
			lpPS->szFullUserTypeNameLSD = (LPSTR)lpLSD+lpLSD->dwFullUserTypeName;
		...
		if (NULL == lpPS->hMetaPictLSD)
		{
			...
			LSTRCPYN(szLabel, lpPS->szSourceOfDataLSD, sizeof(szLabel));
...
void SetPasteSpecialHelpResults(HWND hDlg, LPPASTESPECIAL lpPS)
{
	...
	if (lpPS->dwFlags & PSF_SELECTPASTE)     // If user selected Paste
	{
		if (fIsObject)
		{
			iString = (UINT)(fDisplayAsIcon ? IDS_PSPASTEOBJECTASICON : IDS_PSPASTEOBJECT);
			iImage  = (UINT)(fDisplayAsIcon ? RESULTIMAGE_EMBEDICON   : RESULTIMAGE_EMBED);
	...
	else if (lpPS->dwFlags & PSF_SELECTPASTELINK)   // User selected PasteLink
	{
		if (fIsObject)
		{
			iString = (UINT)(fDisplayAsIcon ? IDS_PSPASTELINKOBJECTASICON : IDS_PSPASTELINKOBJECT);
			iImage  = (UINT)(fDisplayAsIcon ? RESULTIMAGE_LINKICON : RESULTIMAGE_LINK);
		}
		else
		{
		...
		LPMALLOC pIMalloc, LPSTR lpszBuf, LPSTR lpszFullUserTypeName)
{
	...
	if (LB_ERR!=SendMessage(hList,LB_FINDSTRING, 0, (LPARAM)(LPSTR)lpszBuf)) {
