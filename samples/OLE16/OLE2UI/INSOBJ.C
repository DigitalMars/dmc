...
STDAPI_(UINT) OleUIInsertObject(LPOLEUIINSERTOBJECT lpIO)
	...
	if ((lpIO->dwFlags & IOF_SELECTCREATEFROMFILE))
		...
		if (!(lpIO->dwFlags & IOF_CHECKLINK) && (lpIO->dwFlags & IOF_CREATEFILEOBJECT))
			{
			hrErr=OleCreateFromFile(&CLSID_NULL, lpIO->lpszFile, &lpIO->iid
...
static int wGetFileTitle(LPCSTR lpszFile, LPSTR lpszTitle, UINT cbBuf)
{
	...
	if (hinstCommDlg > HINSTANCE_ERROR) {  /* loaded successfully */
...
#else
	return GetFileTitle((LPCSTR)lpszFile, (LPSTR)lpszTitle, cbBuf);
#endif
}
			...
			switch (wID)
				...
				case ID_IO_CHANGEICON:
				{
					...
					if (lpIO->dwFlags & IOF_SELECTCREATENEW)
					{
					   ...
					   pszString = (LPSTR)pIMalloc->lpVtbl->Alloc(pIMalloc, OLEUI_CCHKEYMAX + OLEUI_CCHCLSIDSTRING);
					...
					else  // IOF_SELECTCREATEFROMFILE
					{
					   ...
					   GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, (LPSTR)szFileName, OLEUI_CCHPATHMAX);

					   if (NOERROR != GetClassFile((LPSTR)szFileName, (LPCLSID)&(ci.clsid)))
					   {
						  LPSTR lpszExtension;
						  int   istrlen;
						  ...
						  lpszExtension = (LPSTR)szFileName + istrlen -1;

						  while ( (lpszExtension > szFileName) &&
								  (*lpszExtension != '.') )
							lpszExtension--;

						  GetAssociatedExecutable(lpszExtension, (LPSTR)ci.szIconExe);
					...
					if (0!=uRet)
					{
						...
						if (lpIO->dwFlags & IOF_SELECTCREATENEW)
						  SendMessage(hList, LB_SETITEMDATA, iCurSel, MAKELPARAM(ci.hMetaPict, 0));
					}
				}
					break;
				...
				case ID_IO_FILE:
					...
					nChars = GetDlgItemText(hDlg, ID_IO_FILEDISPLAY, (LPSTR)szTemp, OLEUI_CCHPATHMAX);
					...
					if (FValidateInsertFile(hDlg, FALSE, &lpIO->nErrCode))
					{
						...
						wGetFileTitle((LPSTR)szTemp, lpIO->szFile, OLEUI_CCHPATHMAX);

						istrlen = lstrlen(lpIO->szFile);

						LSTRCPYN((LPSTR)szInitialDir, szTemp, nChars - istrlen);
						fUseInitialDir = TRUE;
						  ...
						  // valid directory
					{
						...
						LSTRCPYN(szBuffer, szTemp, sizeof(szBuffer)-1);
						...
						{
						   lstrcpy(szInitialDir, (LPSTR)szBuffer);
						   fUseInitialDir = TRUE;
						}
#else
					...
					uRet=UStandardHook(lpIO, hDlg, uMsgBrowse
						, OLEUI_CCHPATHMAX, (LPARAM)(LPSTR)lpIO->szFile);
				...
				case IDOK:
				{
					HWND    hListBox;
					WORD    iCurSel;
					char    szBuffer[OLEUI_CCHKEYMAX + OLEUI_CCHCLSIDSTRING];
					LPSTR   lpszCLSID;
   ...
   *szFileType = '\0';
   ...
   if (fSet)
   {
	  ...
	  if (NOERROR == GetClassFile((LPSTR)szFileName, &clsid) )
		OleStdGetUserTypeOfClass(&clsid, szFileType, OLEUI_CCHLABELMAX, NULL);
...
void UpdateClassIcon(HWND hDlg, LPINSERTOBJECT lpIO, HWND hList)
	...
	*pszName=0;
	...
	else
	{
		...
		if (lpIO->dwFlags & IOF_CHECKLINK ||
			...
			hMetaPict = GetIconOfFile(ghInst,
					pszName, lpIO->dwFlags & IOF_CHECKLINK ? TRUE : FALSE);
		} else {
			// not creating a link and we can get CLSID from the filename,
	...
	if (lpIO->dwFlags & IOF_SELECTCREATENEW)
		SendMessage(hList, LB_SETITEMDATA, (WPARAM)iSel, MAKELPARAM(hMetaPict,0));
...
void SetInsertObjectResults(HWND hDlg, LPINSERTOBJECT lpIO)
	...
	cch=512+(UINT)SendDlgItemMessage(hDlg, ID_IO_OBJECTTYPELIST, LB_GETTEXTLEN, i, 0L);
	...
	if (lpIO->dwFlags & IOF_SELECTCREATENEW)
		{
		iString1 = (UINT)(fAsIcon ? IDS_IORESULTNEWICON : IDS_IORESULTNEW);
		iString2 = (UINT)0;
		iImage   = (UINT)(fAsIcon ? RESULTIMAGE_EMBEDICON : RESULTIMAGE_EMBED);
		}
	...
	if (lpIO->dwFlags & IOF_SELECTCREATEFROMFILE)
		...
		if (lpIO->dwFlags & IOF_CHECKLINK)
			{
			iString1 = (UINT)(fAsIcon ? IDS_IORESULTLINKFILEICON1 : IDS_IORESULTLINKFILE1);
			iString2 = (UINT)(fAsIcon ? IDS_IORESULTLINKFILEICON2 : IDS_IORESULTLINKFILE2);
			iImage = (UINT)(fAsIcon ? RESULTIMAGE_LINKICON : RESULTIMAGE_LINK);
			}
		else
			{
			iString1 = (UINT)IDS_IORESULTFROMFILE1;
			iString2 = (UINT)(fAsIcon ? IDS_IORESULTFROMFILEICON2 : IDS_IORESULTFROMFILE2);
			iImage = (UINT)(fAsIcon ? RESULTIMAGE_EMBEDICON : RESULTIMAGE_EMBED);
			}
		}
