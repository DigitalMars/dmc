...
BOOL FBusyInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
	...
	lpBZ=(LPBUSY)LpvStandardInit(hDlg, sizeof(BUSY), TRUE, &hFont);
	...
	*lplpszWindowName = NULL;
	...
	if (hinstToolHelp > HINSTANCE_ERROR) {  /* loaded successfully */
		...
		if (lpfnTaskFindHandle != NULL) {
			...
			fFoundTaskHandle=
					(*lpfnTaskFindHandle) ((TASKENTRY FAR*)&te, (HTASK)htask);
		}
	...
	if (fFoundTaskHandle)
		...
		while (hwndNext)
			...
			if ((hwndNext != hWnd) &&
				...
				if ((lpszTN = OleStdMalloc(OLEUI_CCHPATHMAX)) == NULL)
					return TRUE;  // continue task window enumeration

				if ((lpszWN = OleStdMalloc(OLEUI_CCHPATHMAX)) == NULL)
					return TRUE;  // continue task window enumeration
