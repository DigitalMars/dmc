			...
			switch (wID)
				...
				case ID_LABELEDIT:
					...
					if (EN_KILLFOCUS==wCode)
						{
						GetDlgItemText(hDlg, ID_LABELEDIT, szTemp, sizeof(szTemp));
				...
				case ID_FROMFILEEDIT:
					...
					GetDlgItemText(hDlg, ID_FROMFILEEDIT, szTemp, sizeof(szTemp));
				...
				case ID_BROWSE:
				{
					...
					uRet=UStandardHook(lpCI, hDlg, uMsgBrowse, OLEUI_CCHPATHMAX
					...
					if (0==uRet)
						uRet=(BOOL)Browse(hDlg, lpCI->szFile, NULL, OLEUI_CCHPATHMAX, (UINT)IDS_ICONFILTERS, dwOfnFlags);
...
BOOL FChangeIconInit(HWND hDlg, WPARAM wParam, LPARAM lParam)
	...
	OleUIMetafilePictExtractLabel(lpOCI->hMetaPict, lpCI->szLabel, OLEUI_CCHLABELMAX, NULL);
...
UINT UFillIconList(HWND hDlg, UINT idList, LPSTR pszFile)
	...
	if (HFILE_ERROR!=DoesFileExist(pszFile, &of))
		...
		if (0!=cIcons)
			...
			ErrorWithFile(hDlg, ghInst, (UINT)IDS_CINOICONSINFILE, pszFile, MB_OK);
		}
	else
