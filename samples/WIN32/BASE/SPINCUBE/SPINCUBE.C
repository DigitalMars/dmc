                                  ...
                                  LPARAM lParam)
{
  switch (msg)
  {
    case WM_CREATE:
    {
      ...
      {
        MessageBox (NULL,
                    (LPCTSTR) "SpincubeWndProc(): LocalAlloc() failed",
                    (LPCTSTR) "Err! - SPINCUBE.DLL",
                    MB_OK | MB_ICONEXCLAMATION);
        return -1;
      }
      ...
      {
        MessageBox (NULL,
                    (LPCTSTR) "SpincubeWndProc(): CreateCompatibleDC() failed",
                    (LPCTSTR) "Err! - SPINCUBE.DLL",
                    MB_OK | MB_ICONEXCLAMATION);
        return -1;
      }
    ...
    case WM_SIZE:
      ...
      if (wParam == SIZE_MAXIMIZED || wParam == SIZE_RESTORED)
      {
        ...
        if (!hbmTemp)
        {
          ...
          MessageBox (NULL,
                      (LPCTSTR) "SpincubeWndProc(): CreateCompatibleBitmap() failed",
                      (LPCTSTR) "Err! - SPINCUBE.DLL",
                      MB_OK | MB_ICONEXCLAMATION);
          DestroyWindow (hwnd);
        }

        SelectObject (pSCI->hdcCompat, hbmTemp);
