        ...
        switch (LOWORD(wParam))
          {
            ...
            case IDB_NEXT:
               ...
               if (indexLB == LB_ERR)
                 {
                 MessageBox (hDlg, "Please select an item from the list box",
                             "Registry Monkey Utility", MB_OK);
                 return (0);
                 }
            ...
            case IDB_BACK:
               ...
               if (!hKeyRoot)
                 {
                 MessageBox (hDlg, "Top Level: You can not backup any further.",
                             "Registry Monkey Utility", MB_OK);
                 return (0);
                 }
               ...
               if ((putNullAt = strrchr (RegPath, '\\')) == NULL)
                 {
                 RegPath[0] = '\0';
    ...
    if (*hKeyRoot)
      ...
      if (retCode != ERROR_SUCCESS)
        ...
        if (retCode == ERROR_ACCESS_DENIED)
          wsprintf (Buf, "Error: unable to open key.  Probably due to security reasons.");
        else
          wsprintf (Buf, "Error: Unable to open key, RegOpenKey = %d, Line = %d",
                  retCode, __LINE__);
        ...
        if (dwcValues)
          for (j = 0, retValue = ERROR_SUCCESS; j < dwcValues; j++)
            ...
            if (retValue != (DWORD)ERROR_SUCCESS &&
              ...
              {
              wsprintf (Buf, "Line:%d 0 based index = %d, retValue = %d, ValueLen = %d",
                        __LINE__, j, retValue, dwcValueName);
              MessageBox (hDlg, Buf, "Debug", MB_OK);
              }

            Buf[0] = '\0';
            if (!strlen(ValueName))
              strcpy (ValueName, "<NO NAME>");
...
// ADD A QUERY AND ALLOCATE A BUFFER FOR BDATA.
  ...
  if (retCode != ERROR_SUCCESS)
    ...
    if (dwType < REG_FULL_RESOURCE_DESCRIPTOR)
      {
      wsprintf (Buf, "Error: RegEnumValue = %d, cbData = %d, line %d",
...
//    REG_FULL_RESOURCE_DESCRIPTOR ( 9 )  // Resource list in the hardware description
    ...
    case REG_NONE:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_NONE: No defined value type.");
      break;

    case REG_SZ:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_SZ: A null-terminated Unicode string.");
    ...
    case REG_EXPAND_SZ:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_EXPAND_SZ: A String referencing environment variables i.e. PATH.");
      outBuf = HeapAlloc (hHeap, 0, cbData + 2);
      *outBuf = '\0';
    ...
    case REG_BINARY:
      ...
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_BINARY: Freeform binary data.");
      SetCursor (LoadCursor (NULL, IDC_WAIT));
      ...
      if (BinaryStrBuf)
        ...
        *ByteBuf = '\0';
        ...
        {
        MessageBox (hDlg, "Error: BinaryStrBuf = malloc failed",
                    "Debug: DisplayKeyData", MB_OK);
        }
    ...
    case REG_DWORD:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_DWORD: A 32 bit number.");
      SetDlgItemInt (hDlg, IDE_VALUE2, *(UINT *)bData, FALSE);
      break;

    case REG_DWORD_BIG_ENDIAN:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_DWORD_BIG_ENDIAN: A 32 bit number in big endian format.");
      SetDlgItemInt (hDlg, IDE_VALUE2, *(UINT *)bData, TRUE);
      break;

    case REG_LINK:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_LINK: A Unicode symbolic link.");
      SetDlgItemText (hDlg, IDE_VALUE2, bData);
      break;

    case REG_MULTI_SZ:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_MULTI_SZ: An array of null-terminated strings.");
      SetCursor (LoadCursor (NULL, IDC_WAIT));
    ...
    case REG_RESOURCE_LIST:            // CM_RESOURCE_LIST is kind of complex,
                                       ...
                                       // different release.
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_RESOURCE_LIST: A device-driver resource list.");
    ...
    case REG_FULL_RESOURCE_DESCRIPTOR:
      SetDlgItemText (hDlg, IDE_VALUE1, "REG_FULL_RESOURCE_DESCRIPTOR: A resource list in the hardware description.");
      break;



    default:
      wsprintf (Buf, "Undefine in this verion of the Registry Monkey. %d",
               dwType);
