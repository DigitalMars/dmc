...
#include "precomp.h"
#include <oledlg.h>     // for OleUIInsertObject and OleUIAddVerbMenu
#include "app.h"            
#include "site.h"
#include "doc.h"
...
// Parameters:
//                                                                     
//      CSimpleApp *lpApp       -   Pointer to the CSimpleApp Class
//
...
CSimpleDoc* CSimpleDoc::Create(CSimpleApp *lpApp, LPRECT lpRect,HWND hWnd)
{   
    CSimpleDoc *lpTemp = new CSimpleDoc(lpApp, hWnd);
    ...
    lpTemp->m_hDocWnd = CreateWindow(TEXT("SimpCntrDocWClass"),
                                     NULL,
...
{   
    OutputDebugString(TEXT("In CSimpleDoc::Close\r\n"));
...
CSimpleDoc::CSimpleDoc(CSimpleApp *lpApp, HWND hWnd)
{   
    OutputDebugString(TEXT("In CSimpleDoc's Constructor\r\n")); 
...
CSimpleDoc::~CSimpleDoc()
{      
    OutputDebugString(TEXT("In CSimpleDoc's Destructor\r\n"));
...
//      LPVOID* ppvObj      -   Location to return the interface
// 
// Return Value:
//                                                              
//      S_FALSE -   Always
...
STDMETHODIMP CSimpleDoc::QueryInterface(REFIID riid, LPVOID* ppvObj)
{
    OutputDebugString(TEXT("In CSimpleDoc::QueryInterface\r\n"));
...
{
    OutputDebugString(TEXT("In CSimpleDoc::AddRef\r\n"));
...
{
    OutputDebugString(TEXT("In CSimpleDoc::Release\r\n"));
...
void CSimpleDoc::InsertObject() 
{
    ...
    io.ppvObj           = (LPVOID*)&m_lpSite->m_lpOleObject;
    m_lpSite->QueryInterface(IID_IOleClientSite, (LPVOID*)&io.lpIOleClientSite);

    // let the system do all the hard work                     
    iret = OleUIInsertObject(&io);      

    io.lpIOleClientSite->Release(); // done with this now
...
{ 
    ...
    if (m_fInPlaceActive)
        m_lpActiveObject->ResizeBorder(lpRect, (LPOLEINPLACEUIWINDOW)m_lpApp, TRUE);
    else
