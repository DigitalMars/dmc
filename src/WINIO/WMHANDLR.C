...
/* -----------------------------------------------------------------------  */
BOOL wmhandler_isextramsg(WMTAB wmTab, WORD wMsg, WORD *pwExtraMsg)
	{
	WORD i;
...
/* -----------------------------------------------------------------------  */
long FAR PASCAL _export wmhandler_wndproc(HWND hWnd, WORD wMsg,
...
/* ----------------------------------------------------------------------- */
long defwmhandler(HWND hwnd, unsigned wMsg, WORD wParam, LONG lParam)
    {   
    return DefWindowProc(hwnd, wMsg, wParam, lParam);
    }
...
WMTAB wmhandler_create(void)
	...
	if ((wmTab = (WMTAB) GlobalLock(
						GlobalAlloc(GMEM_MOVEABLE, sizeof(WMHANDLERDATA))))==NULL)
		return NULL;
