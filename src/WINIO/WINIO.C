...
long            winio_wmcommand(HWND, unsigned, WORD, LONG);
...
long            winio_wmldblclk(HWND, unsigned, WORD, LONG);
...
BOOL winio_init(void)
	...
	if ((! __hPrevInst) && (! initialize_class(__hInst)))
		{
		winio_warn(FALSE, (char *)winio_error,"Could not create class");
		return FALSE;
		}
	...
	strcpy((char *)winio_deftitle, __szModule);
//	GetModuleFileName(__hInst, winio_deftitle, 128); // Gets unwieldy!
	if (__lpCmdLine && *__lpCmdLine)
		{
		_fstrcat((char *)winio_deftitle, " ");
		_fstrcat((char *)winio_deftitle, __lpCmdLine);
		}
...
HWND winio_window(LPSTR strTitle, WORD wBufSize, WORD wFlags)
    ...
    if ((hWnd = new_window((LPCREATEPARAMS) &cp,
			strTitle ? strTitle : (char *)winio_deftitle,
			wFlags)) == NULL)
        return NULL;
...
int initialize_class(HANDLE hInst)
    ...
    wc.style = CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS
				/* | CS_BYTEALIGNCLIENT */;
    wc.lpfnWndProc = (WNDPROC)wmhandler_wndproc;
    wc.cbClsExtra = 0;
    wc.cbWndExtra = 8;
    wc.hInstance = hInst;
...
BOOL initialize_buffers(WINIO_HWND whWnd, WORD wBufSize, WORD wFlags)
	...
	if ((whWnd->hBuffer =
			GlobalAlloc(GMEM_MOVEABLE, (DWORD) whWnd->bufsize))==NULL)
		return FALSE;
	...
	if (! (wFlags & WW_OUTPUTONLY))
		{
		if ((whWnd->hKeyboard = GlobalAlloc(GMEM_MOVEABLE, TYPE_AHEAD))==NULL)
			{
...
long winio_wmpaint(HWND hwnd, unsigned message, WORD wParam, LONG lParam)
    ...
    BYTE far *pchSOL = (BYTE far *)whWnd->fpTopOfWin;
	...
	for (i = 0; i < yTop; i++)      // lines above repaint region
		{
		while ((pchSOL < (BYTE far *)whWnd->fpCurrLine) && *pchSOL)
			pchSOL++;
		if (pchSOL < (BYTE far *)whWnd->fpCurrLine)
			pchSOL++;
		else
			break;
		}
	...
	if ((i == yTop) && (i <= whWnd->yCurrLine)) // something needs repainting
		...
		for (; i <= yBottom; i++)   // lines in repaint region
			...
			TextOut(hdc, whWnd->cxChar * xStart, whWnd->cyChar * i,
				(LPCSTR)pchSOL, min(j, xRight - xLeft + 1));
			if ((unsigned) (pchEOL - (BYTE far *)whWnd->fpBuffer) >= whWnd->bufused)
				break;
			pchSOL = ++pchEOL;  
			}
		}
...
long winio_wmchar(HWND hwnd, unsigned message, WORD wParam, LONG lParam)
    {
	WINIO_HWND whWnd = (WINIO_HWND) GetWindowLong(hwnd, 4);
    BYTE far *lpchKeybd = (BYTE far *)whWnd->fpKeyboard;
...
long winio_wmldblclk(HWND hwnd, unsigned message, WORD wParam, LONG lParam)
	...
	for (; yRel > 0; yRel--)
		lpLine = (LPSTR)nextline((BYTE far *)lpLine);
...
long winio_wmvscroll(HWND hwnd, unsigned message, WORD wParam, LONG lParam)
    ...
    if (whWnd->yTopOfWin > cySave)
        for (i = cySave; i < whWnd->yTopOfWin; i++)
            whWnd->fpTopOfWin = (LPSTR)nextline((BYTE far *)whWnd->fpTopOfWin);
    else
        for (i = cySave; i > whWnd->yTopOfWin; i--)
            whWnd->fpTopOfWin = (LPSTR)prevline((BYTE far *)whWnd->fpTopOfWin);
...
void compute_repaint(void)
    ...
    if (whwndCurr->yCurrLine > (whwndCurr->yTopOfWin + whwndCurr->yWinHeight))
        ...
        while (whwndCurr->yTopOfWin <
            ...
            {
            whwndCurr->fpTopOfWin = (LPSTR)nextline((BYTE far *)whwndCurr->fpTopOfWin);
            whwndCurr->yTopOfWin++;
            }
        tWholeWin = TRUE;
        }
...
void make_avail(unsigned cch)
    ...
    fpNew = (BYTE far *)whwndCurr->fpBuffer;
    fpNew += (LONG) cDiscard;
    fpTmp = (BYTE far *)whwndCurr->fpBuffer;
	for (yDrop = 0; fpTmp < fpNew; fpTmp = nextline(fpTmp), yDrop++) ;
    cDiscard = fpTmp - (BYTE far *)whwndCurr->fpBuffer; 
